...
#ifdef SUNOS
extern int fclose(VIRTUAL_FILE *);
extern int fgetc(VIRTUAL_FILE *);
extern int fputc(int, VIRTUAL_FILE *);
extern size_t fread(void *, size_t, size_t, VIRTUAL_FILE *);
extern int fseek(VIRTUAL_FILE *, long, int);
extern size_t fwrite(const void *, size_t, size_t, VIRTUAL_FILE *);
#endif
...
VIRTUAL_FILE *_mm_fopen(CHAR *a, CHAR *b)
{
	return NULL;
}
...
{	
	uint32_t fpos = ((MFILEREADER*)reader)->file->pos;
	uint32_t fsize = ((MFILEREADER*)reader)->file->size;
